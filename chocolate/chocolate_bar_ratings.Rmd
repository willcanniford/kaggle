---
title: "Chocolate Bar Ratings: Data Visualisations"
...
The data here is provided in a csv file and kept in the input directory, which is inside the current working directory. I just simply load it here using the `read_csv` function. 
...
# Remove that first row that seems to be a duplication of the headers
...
Removing the % sign from the cocoa percent column allows us to convert it to a numeric type which will help in further analysis

```{r}
# Have a look at the summary of the new data with the new column types
summary(full_data)
```
...
```{r}
# Mutate a new column that converts the year into a date type 
full_data <- full_data %>%
  mutate(review_date = ymd(paste(review_date, 1, 1, sep="-")))

# Generate a table of average ratings by year
...
Distribution of ratings faceted by year
```{r}
...
We seem to see that the standard deviation of rating seems to be decreasing as time passes. 
...
```{r}
low_scores <- full_data %>% group_by(review_date) %>%
...
low_scores %>%
  ...
  labs(x = 'Date', 
       y = 'Percentage',
       title = 'Percentage of scores below 2.5 by year')

```
  
### Ratings over time
Are reviewers getting more generous in recent years?
```{r}
full_data %>%
  ggplot(aes(review_date, rating)) + 
  geom_count(aes(colour = ..n..)) + 
  geom_line(data = avg_ratings, aes(review_date, avg_rating), colour = 'red') +
  scale_x_date(date_breaks = '1 year', date_labels = '%Y') + 
  scale_size_continuous(name = 'Count') + scale_color_continuous(name = 'Count') + 
  coord_cartesian(ylim = c(0,5)) + 
  theme_minimal() + labs(x = 'Review year', y = 'Rating')
```

Reviewers don't appear to be more generous, or as harsh, considering our previous analysis. 
  
### Company location
Does the company location make a difference to the quality of the product? 
```{r message = FALSE}
full_data %>%
  group_by(company_location) %>% filter(n() > 10) %>% mutate(avg = mean(rating)) %>%
  ggplot() + 
  geom_boxplot(aes(reorder(company_location, avg), rating, fill = avg)) + 
  scale_fill_continuous(low = '#ffffcc', high = '#fc4e2a') + 
  coord_flip() + 
  theme_minimal() + 
...
### Master makers
Are there masterful makers that can do no wrong? 
```{r warning = FALSE, message=FALSE}
# Split the maker from the company_maker column before choosing those makers that have at least 3 reviewed products
colnames(full_data)[1] <- 'company_maker'
makers <- full_data %>%
  rowwise %>%
  mutate(maker = str_match(company_maker, '\\((.*)\\)')[2]) %>% 
  filter(!is.na(maker)) %>% group_by(maker) %>% filter(n() >= 3) %>% mutate(avg = mean(rating)) 
...
# I split the company/maker column again and then filter by companies that have at least 10 reviews to their name
companies <- full_data %>%
  rowwise() %>%
...
# By plotting the count of reviews for certain values we are also able to see the distribution of 
# scores that a particular company has obtained 
companies  %>%
...
full_data %>%
  ...
  ggplot() + 
    ...
    guides(colour = FALSE) + labs(x = 'Year of review', y = 'Average rating') + 
...
Thanks for taking the time to have a look at my work. Any comments/feedback welcome. 